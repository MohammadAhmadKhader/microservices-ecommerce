...
function createBaseEmptyBody(): EmptyBody {
  return {};
}

export const EmptyBody: MessageFns<EmptyBody> = {
  encode(_: EmptyBody, writer: BinaryWriter = new BinaryWriter()): BinaryWriter {
    return writer;
  },

  decode(input: BinaryReader | Uint8Array, length?: number): EmptyBody {
    const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
    let end = length === undefined ? reader.len : reader.pos + length;
    const message = createBaseEmptyBody();
    while (reader.pos < end) {
      const tag = reader.uint32();
      switch (tag >>> 3) {
      }
  ...
  fromJSON(_: any): EmptyBody {
    return {};
  },

  toJSON(_: EmptyBody): unknown {
    const obj: any = {};
    return obj;
  },

  create<I extends Exact<DeepPartial<EmptyBody>, I>>(base?: I): EmptyBody {
    return EmptyBody.fromPartial(base ?? ({} as any));
  },
  fromPartial<I extends Exact<DeepPartial<EmptyBody>, I>>(_: I): EmptyBody {
    const message = createBaseEmptyBody();
    return message;
  },
};
